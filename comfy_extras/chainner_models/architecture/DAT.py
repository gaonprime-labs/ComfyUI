...
class SpatialGate(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        B, N, C = x.shape
        x2 = (self.conv(self.norm(x2).transpose(1, 2).contiguous().view(B, C//2, H, W)).flatten(2).transpose(
            -1, -2).contiguous())
...
class Adaptive_Spatial_Attention(nn.Module):
    ...
    def __init__(
        ...
        if (self.rg_idx % 2 == 0 and self.b_idx > 0 and
            (self.b_idx - 2) % 4 == 0) or (self.rg_idx % 2 != 0 and self.b_idx % 4 == 0):
    ...
    def calculate_mask(self, H, W):
        ...
        img_mask_0 = (img_mask_0.permute(0, 1, 3, 2, 4,
                                         5).contiguous().view(-1, self.split_size[0], self.split_size[1],
                                                              1))  # nW, sw[0], sw[1], 1
        mask_windows_0 = img_mask_0.view(-1, self.split_size[0]*self.split_size[1])
        attn_mask_0 = mask_windows_0.unsqueeze(1) - mask_windows_0.unsqueeze(2)
        attn_mask_0 = attn_mask_0.masked_fill(attn_mask_0 != 0,
                                              float(-100.0)).masked_fill(attn_mask_0 == 0, float(0.0))
        ...
        img_mask_1 = (img_mask_1.permute(0, 1, 3, 2, 4,
                                         5).contiguous().view(-1, self.split_size[1], self.split_size[0],
                                                              1))  # nW, sw[1], sw[0], 1
        mask_windows_1 = img_mask_1.view(-1, self.split_size[1]*self.split_size[0])
        attn_mask_1 = mask_windows_1.unsqueeze(1) - mask_windows_1.unsqueeze(2)
        attn_mask_1 = attn_mask_1.masked_fill(attn_mask_1 != 0,
                                              float(-100.0)).masked_fill(attn_mask_1 == 0, float(0.0))
...
class DAT(nn.Module):
    ...
    def __init__(self, state_dict):
        ...
        num_feat = (state_dict.get("conv_before_upsample.0.weight", None).shape[1] if state_dict.get(
            "conv_before_upsample.weight", None) else 64)
        ...
        if "layers.0.blocks.2.attn.attn_mask_0" in state_keys:
            attn_mask_0_x, attn_mask_0_y, attn_mask_0_z = state_dict[
                "layers.0.blocks.2.attn.attn_mask_0"].shape
        ...
        curr_dim = embed_dim
        dpr = [x.item()
               for x in torch.linspace(0, drop_path_rate, np.sum(depth))]  # stochastic depth decay rule
