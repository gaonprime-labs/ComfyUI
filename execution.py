...
def get_input_data(inputs, class_def, unique_id, outputs={}, prompt={}, extra_data={}):
    ...
    for x in inputs:
        ...
        if isinstance(input_data, list):
            ...
            if ("required" in valid_inputs
                    and x in valid_inputs["required"]) or ("optional" in valid_inputs
                                                           and x in valid_inputs["optional"]):
...
def recursive_execute(server, prompt, outputs, current_item, extra_data, executed, prompt_id, outputs_ui,
    ...
    for x in inputs:
        ...
        if isinstance(input_data, list):
            ...
            output_index = input_data[1]
            if input_unique_id not in outputs:
                result = recursive_execute(server, prompt, outputs, input_unique_id, extra_data, executed,
            ...
            for x in inputs:
                ...
                if isinstance(input_data, list):
                    ...
                    if input_unique_id in outputs:
                        to_delete = recursive_output_delete_if_changed(prompt, old_prompt, outputs,
                                                                       input_unique_id)
                    else:
                        to_delete = True
                    if to_delete:
...
class PromptExecutor:
    ...
    def execute(self, prompt, prompt_id, extra_data={}, execute_outputs=[]):
        ...
        with torch.inference_mode():
            ...
            if self.server.client_id is not None:
                self.server.send_sync("execution_cached", {
            ...
            while len(to_execute) > 0:
                ...
                to_execute = sorted(
                    list(
                        map(lambda a: (len(recursive_will_execute(prompt, self.outputs, a[-1])), a[-1]),
                            to_execute)))
                ...
                # error was raised
                success, error, ex = recursive_execute(self.server, prompt, self.outputs, output_node_id,
