...
class LoadedModel:
    ...
    def model_load(self, lowvram_model_memory=0):
        ...
        if is_intel_xpu() and not args.disable_ipex_optimize:
            self.real_model = torch.xpu.optimize(self.real_model.eval(), inplace=True,
...
def load_models_gpu(models, memory_required=0):
    ...
    for loaded_model in models_to_load:
        ...
        if lowvram_available and (vram_set_state == VRAMState.LOW_VRAM
                                  or vram_set_state == VRAMState.NORMAL_VRAM):
            model_size = loaded_model.model_memory_required(torch_dev)
            current_free_mem = get_free_memory(torch_dev)
            lowvram_model_memory = int(max(64*(1024*1024), (current_free_mem - 1024*(1024*1024))/1.3))
            if model_size > (current_free_mem -
                             inference_memory):  #only switch to lowvram if really necessary
    ...
    if device_supports_cast:
        if copy:
            ...
            return tensor.to(device, copy=copy,
                             non_blocking=non_blocking).to(dtype, non_blocking=non_blocking)
        else:
...
def should_use_fp16(device=None, model_params=0, prioritize_performance=True):
    ...
    nvidia_10_series = [
        "1080", "1070", "titan x", "p3000", "p3200", "p4000", "p4200", "p5000", "p5200", "p6000", "1060",
        "1050"
    ]
    ...
    nvidia_16_series = [
        "1660", "1650", "1630", "T500", "T550", "T600", "MX550", "MX450", "CMP 30HX", "T2000", "T1000",
        "T1200"
    ]
