...
class DiffusersLoader:
    ...
    def load_checkpoint(self, model_path, output_vae=True, output_clip=True):
        ...
        return comfy.diffusers_load.load_diffusers(
            model_path, output_vae=output_vae, output_clip=output_clip,
            embedding_directory=folder_paths.get_folder_paths("embeddings"))
...
class unCLIPCheckpointLoader:
    ...
    def load_checkpoint(self, ckpt_name, output_vae=True, output_clip=True):
        ckpt_path = folder_paths.get_full_path("checkpoints", ckpt_name)
        out = comfy.sd.load_checkpoint_guess_config(
            ckpt_path, output_vae=True, output_clip=True, output_clipvision=True,
            embedding_directory=folder_paths.get_folder_paths("embeddings"))
        return out
...
class LoraLoader:
    ...
    def load_lora(self, model, clip, lora_name, strength_model, strength_clip):
        ...
        model_lora, clip_lora = comfy.sd.load_lora_for_models(model, clip, lora, strength_model,
                                                              strength_clip)
...
class LatentUpscale:
    ...
    def upscale(self, samples, upscale_method, width, height, crop):
        if width == 0 and height == 0:
            ...
            s["samples"] = comfy.utils.common_upscale(samples["samples"], width//8, height//8, upscale_method,
                                                      crop)
        return (s, )
...
class LatentUpscaleBy:
    ...
    def upscale(self, samples, upscale_method, scale_by):
        ...
        s["samples"] = comfy.utils.common_upscale(samples["samples"], width, height, upscale_method,
                                                  "disabled")
        return (s, )
...
class LatentComposite:
    ...
    def composite(self, samples_to, samples_from, x, y, composite_method="normal", feather=0):
        ...
        if feather == 0:
            s[:, :, y:y + samples_from.shape[2], x:x +
              samples_from.shape[3]] = samples_from[:, :, :samples_to.shape[2] - y, :samples_to.shape[3] - x]
        else:
            ...
            s[:, :, y:y + samples_from.shape[2], x:x +
              samples_from.shape[3]] = samples_from[:, :, :samples_to.shape[2] - y, :samples_to.shape[3] -
                                                    x]*mask + s[:, :, y:y + samples_from.shape[2],
                                                                x:x + samples_from.shape[3]]*rev_mask
...
class KSampler:
    ...
    def sample(self, model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent_image,
               denoise=1.0):
        return common_ksampler(model, seed, steps, cfg, sampler_name, scheduler, positive, negative,
...
def load_custom_node(module_path, ignore=set()):
    ...
    try:
        ...
        if hasattr(module, "NODE_CLASS_MAPPINGS") and getattr(module, "NODE_CLASS_MAPPINGS") is not None:
            ...
            if hasattr(module, "NODE_DISPLAY_NAME_MAPPINGS") and getattr(
                    module, "NODE_DISPLAY_NAME_MAPPINGS") is not None:
